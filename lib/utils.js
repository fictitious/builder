...
function getCanonicalNamePlain(loader, normalized, isPlugin) {
  ...
  if (!pathMatch)
    for (var p in loader.paths) {
      ...
      // normalize the output path. Always disable defaultJSExtensions for wildcard matches - see #413
...
var absURLRegEx = /^[^\/]+:\/\//;
function normalizePath(loader, path, disableDefaultJSExtensions) {
  var curPath;
  ...
  if (loader.defaultJSExtensions && !disableDefaultJSExtensions && curPath.substr(curPath.length - 3, 3) != '.js')
    curPath += '.js';
  return curPath;
}
